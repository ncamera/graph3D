import { BufferGeometry, Group, Line, LineBasicMaterial, Sprite, SpriteMaterial, Texture, Vector3 } from "three";
// - - - - - - - - - - - - - Constantes - - - - - - - - - - - - -
...
function setScale(axes) {
    ...
    for (i = 0; i < xPoints.length; i++) {
        this.group.add(drawLine(xPoints[i], xPoints[i], axes.zMin, axes.zMax, axes.xMin, axes.xMin), drawLine(xPoints[i], xPoints[i], axes.zMin, axes.zMin, axes.xMin, axes.xMax + AXE_TICK_PADDING), drawTickLabel(xPoints[i], axes.zMin, axes.xMax + AXE_LABEL_PADDING, xPoints[i].toFixed(0)));
    }
    for (i = 0; i < yPoints.length; i++) {
        this.group.add(drawLine(axes.yMin, axes.yMax, yPoints[i], yPoints[i], axes.xMin, axes.xMin), drawLine(axes.yMin, axes.yMin, yPoints[i], yPoints[i], axes.xMin, axes.xMax));
        if (i !== 0) {
            this.group.add(drawLine(axes.yMin - AXE_TICK_PADDING, axes.yMin, yPoints[i], yPoints[i], axes.xMax, axes.xMax), drawTickLabel(axes.yMin - AXE_LABEL_PADDING, yPoints[i], axes.xMax, yPoints[i].toFixed(0)));
        }
    }
    for (i = 0; i < zPoints.length; i++) {
        this.group.add(drawLine(axes.yMin, axes.yMax + AXE_TICK_PADDING, axes.zMin, axes.zMin, zPoints[i], zPoints[i]), drawLine(axes.yMin, axes.yMin, axes.zMin, axes.zMax, zPoints[i], zPoints[i]));
        if (i !== 0) {
            this.group.add(drawTickLabel(axes.yMax + AXE_LABEL_PADDING, axes.zMin, zPoints[i], zPoints[i].toFixed(0)));
        }
    }
}
